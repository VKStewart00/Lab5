...
teacherE <- read_csv("teacher_evals.csv")
...
>> Data consist of teacher ID, number of questions, number of participants, teachers score, and various information on students' academics.  

**4. What is the unit of observation (i.e. a single row in the dataset) identified by?**

>> There is a teacher ID number, along with a course ID number.  
...
## load the data
teacher_evals_clean <- teacherE|> 
  
  ## Rename the 'sex' column to gender
  rename( sex = gender )|>
  
  ## Removes courses with fewer then 10 ppl
  filter(no_participants >= 10)|>
  
  ## Sets teacher id to numeric
  mutate(teacher_id = as.numeric(teacher_id))|>
  
  ## Selects for the columns we want
  ...
  ## Saves the csv
  write_csv("TeacherEC.csv")
  
## Checking my work
view(teacher_evals_clean)
...
>> There is 939 unique course IDs, and 297 teacher IDs.  
...
print(teacher_evals_clean|>
  ...
  ## checks across all columns for NA values.  
    ## got the idea for this from here: https://stackoverflow.com/questions/66572756/checking-all-columns-in-data-frame-for-missing-values-in-r
  filter(if_any(everything(), is.na))) 

```
>> teacher number 56347, they are missing `percent_failed_cur` column.  
...
## Determine distribution of academic_degree
ggplot(teacher_evals_clean, aes(x = academic_degree)) +
  geom_bar() +
  labs(title = "Distribution of Academic Degrees")

## Determine distribution of seniority
ggplot(teacher_evals_clean, aes(x = seniority)) +
  geom_boxplot() +
  labs(title = "Distribution of Seniority")

## Determine distribution of sex
...
>> There are more male than female teachers in this study.  The majority of these teachers have a doctorate.  And 75% of them have been teaching for less than six years.  
...
teacher_evals_clean |> 
  ...
  ## counts how many times each teacher and course ID exist in the dataset
  count(teacher_id, course_id)|>
  
  ## filters out how many have more than nine responses
  filter(n == 9 )

```
>> 49 combinations asked all nine questions.  
...
>> Teacher 3477 has the lowest score, while teacher 66074 has the higest score.  
...
teacher_evals_clean|>
  ...
  ## Selects for teachers with one year experience
  filter( seniority == 1)|>
  
  ## Groups by teacher ID
  group_by( teacher_id )|>
  
  ## Sorts percent failed
  arrange(percent_failed_cur)

```

>> Teacher 98650 has the lowest fail rate, while teacher 104362 has the highest.
...
teacher_evals_clean|>
  ...
  filter( sex == "female" , academic_degree == c("dr", "prof"))|>
...
``` 
 ...
 >> Teacher 3485 has the lowest fail rate, while teacher 43811 has the higest failure rate.   
